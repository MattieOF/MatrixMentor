...
struct Transform
{
    ...
    FORCEINLINE void Rotate(glm::vec3 rotation) { Rotation += rotation; } // TODO: It's not this simple!
    FORCEINLINE void AddScale(glm::vec3 scale) { Scale += scale; }
    FORCEINLINE void ScaleBy(glm::vec3 scale) { Scale *= scale; }
    
    FORCEINLINE glm::mat4 GetTransformationMatrix() const { return MathUtil::CreateTransformationMatrix(Position, Rotation, Scale); }
